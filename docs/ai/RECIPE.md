# visionOS RTC quickstart

## Overview

Create a visionOS quickstart that joins an Agora RTC channel for immersive real-time communication.

Use this recipe when you want a working reference for visionOS, RTC channel, spatial app, quickstart.

Repository: [AgoraIO-Community/visionOS-Quickstart](https://github.com/AgoraIO-Community/visionOS-Quickstart)
...
Keep production Agora tokens server-generated and renew them before expiration.

## Run The Sample

Follow the repository README to install dependencies, configure the Agora App ID, then run the sample and join a test channel from another client to verify media flow.
...
```text
You are working in `AgoraIO-Community/visionOS-Quickstart`. Add or adapt this sample so a developer can run the visionos rtc quickstart end to end with Agora RTC.
